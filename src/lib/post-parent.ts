...
export const tools: ToolIntro[] = [
  // ── ① 聞く ─────────────────────────────────────────────
  {
    ...
    chips: ['新しい面談枠は不要', '初回は3つだけ', 'モニタリングは毎回同じ3問', '印刷して使えます'],
    body:
      '新しい面談枠は作りません。初回のアセスメント、モニタリングの訪問、担当者会議、更新の手続き——' +
      'すでにある場面に、問いをいくつか足すだけのガイドです。' +
      '初回は命に関わる3つ（してはいけないこと・落ち着くまでの順序・期限のあるもの）だけ。' +
      '中心はモニタリングの3問で、「前の対応は今も同じか・効いたか」「要らなくなったことは」「新しく気づいたことは」を毎回置くだけで、情報が古くならずに済みます。' +
      '新しいことを聞き出すより、すでに聞いたことを確かめ直すほうが、実は価値が高い——それがこのガイドの中心です。',
    links: [
  ...
  // ── ③ 引き出す ───────────────────────────────────
  {
    ...
    chips: ['GitHubで無料公開（MIT）', 'Mac・Windows対応', '中核は AI なしで動きます', 'スマホから記録入力'],
    body:
      ...
      'しくみ全体は「oya-inai-db」としてGitHubで無料公開（MITライセンス）しています。有料ソフトの購入は不要で、' +
      '中核機能（緊急時の禁忌表示・期限管理）は AI なしで動きます。' +
      '一方、上の「渡す」の仕組み（語りの振り分け）を使う場合は、AI サービスの有料プランが必要です。' +
  ...
  },
  {
    ...
    body:
      ...
      '聴き取りで挙がった「もしものとき、いちばん先に連絡する人」や、いま関わっている事業所・医療機関が、そのまま図になります。',
