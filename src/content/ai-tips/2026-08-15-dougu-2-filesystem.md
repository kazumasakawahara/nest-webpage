...
<div class="callout">
  <span class="callout__label">2026年9月の更新 ― もっとかんたんになりました</span>
  <p>2026年9月から、Claudeの「チャット」と、作業をまかせる「Cowork（コワーク）」がひとつにまとまり、<strong>Claude Desktop そのものに、フォルダへのアクセスを許可できる</strong>ようになりました。新しい画面なら、この記事の②〜④のように<strong>Filesystem のコネクタを探して入れる必要はありません</strong>。やり方は<a href="#new-screen">下の「新しい画面の場合」</a>をご覧ください。</p>
  <p>ただし、新しい画面は<strong>有料の Pro・Max プランから順番に届いている途中</strong>です（無料プランには「もうすぐ」とされています）。まだ画面が変わっていない方は、これまでどおり①〜⑤の手順で大丈夫です。<strong>「専用フォルダを1つだけ見せる」という考え方は、どちらの画面でも変わりません。</strong></p>
</div>

→ コネクタとは何か、なぜAIと“共有フォルダ”を持つと良いのか——その総論は、連載・実践編の [AIと“申し送りノート”を持つ ― 共有フォルダで、忘れないAIに](/ai-tips/2026-07-16-ai-shared-workspace/) でお話ししました。この記事は、その中の Filesystem に絞った手順編です。ほかのコネクタも、同じ要領（カタログで探す→提供元を確かめる→追加する）で差せます。
...
<h2 id="new-screen">新しい画面の場合 ― フォルダを「許可」するだけ（2026年9月〜）</h2>

新しい画面では、コネクタのカタログを開く必要がありません。AIに、**直接フォルダを見せる**流れになりました。

### ① 専用フォルダを用意する

ここは前半の①と同じです。書類フォルダの中に「AI作業室」など、AIに見せる専用のフォルダを1つ作ります。

### ② 「プロジェクトまたはフォルダー」から「フォルダを追加」

チャットの入力欄のすぐ下に、「**プロジェクトまたはフォルダー**」というボタンがあります。押すと一覧が開くので、いちばん下の「**フォルダを追加**」を選びます。

<figure class="mockup mockup--shot mockup--wide">
  <img src="/images/ai-tips/dougu-2-folder-add-menu.webp" alt="Claude Desktopのチャット入力欄。下にある「プロジェクトまたはフォルダー」ボタンを押すと一覧が開き、プロジェクトの検索欄、ピン留め済みのプロジェクト、その下に「新しいプロジェクト」と「フォルダを追加」が並んでいる" width="1310" height="948" loading="lazy" decoding="async" />
  <figcaption>▲ 入力欄の下の「プロジェクトまたはフォルダー」→ いちばん下の「フォルダを追加」</figcaption>
</figure>

すると、Macならいつもの「ファイルを開く」ときと同じフォルダ選びの画面が出てきます（Windowsでも、いつものフォルダ選びの画面です）。①の専用フォルダを選んで「**開く**」を押してください。

### ③ 確認の画面を、よく読んでから許可する

最後に、こんな確認が出ます。ここが、この節でいちばん大事なところです。

<figure class="mockup mockup--shot">
  <img src="/images/ai-tips/dougu-2-folder-allow-dialog.webp" alt="確認の画面。「Claudeが『my-research』内のファイルを変更することを許可しますか？」「Claudeは接続されたツールを使用して、これらのファイルを編集、削除、共有できます。」と書かれ、キャンセル・常に許可・許可の3つのボタンが並んでいる" width="835" height="510" loading="lazy" decoding="async" />
  <figcaption>▲ 「編集、削除、共有できます」とはっきり書いてあります（画面の例では別のフォルダを選んでいます）</figcaption>
</figure>

「**編集、削除、共有できます**」——つまり、許可したフォルダの中では、AIがファイルを書き換えたり消したりできる、ということです。だからこそ、①で、**専用のフォルダ**を作ったのでした。ボタンは3つあります。

- **許可** —— ひとまず許可します。慣れるまでは、こちらがおすすめです
- **常に許可** —— 次からは、この確認を出さずに使わせる設定です。使い慣れて、中身を任せてよいと思えてから選んでも遅くありません
- **キャンセル** —— やめます。フォルダの選び間違いに気づいたら、迷わずこちらへ

<div class="callout">
  <span class="callout__label">気をつけたいこと ― 手が届くのは「アプリが開いている間」だけ</span>
  <p>AIがあなたのパソコンのフォルダに触れられるのは、<strong>Claude Desktop が起動している間だけ</strong>です。スマホやブラウザから同じ会話の続きを頼んでも、パソコンのアプリが閉じていれば、フォルダには届きません。「頼んだのに読んでくれない」ときは、まずパソコンのアプリが開いているかを確かめてください。</p>
</div>

確かめ方は、これまでと同じです。「『AI作業室』フォルダの中に、いま何がありますか？」と聞いて、中身を答えてくれたら成功です。
