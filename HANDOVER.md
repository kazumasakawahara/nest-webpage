...
> 河原さん発案の常設コーナー。**うまくいった頼み方を、そのときの文面と返ってきたものごと一つずつ棚に並べる**。その1は「図（HTMLが良い）」の一言で、パソコン版 Google ドライブの同期方式の説明が、ボタンで切り替わる一枚のページになった実例。依頼文は河原さんの原文、お手本 HTML は AI 出力を無加工で同梱。**その1〜5 を PR #40〜#43 で公開済み（2026-09-11）。次は D グラフから。候補は docs/ai-tips-kikaku-2026-09-11-onegai-ideas.md（A〜G＋締めの H「重なったら Skill に」）。**
...
- [x] その2〜4（B クイズ／C 紙芝居／A 掲示物）を実演・執筆。A は一回目がはみ出し→差し戻しで収まった実録。**PR #41 で main にマージ・本番確認済み（2026-09-11）**
- [x] その5「お願いの合わせ技」（河原さん提供の依頼文2本＝成年後見トピック記事の実録）＋看板挿絵の修正。PR #42 で公開、タイトルは河原さん指示で「nest-news トピックコーナーの成年後見制度改正の解説ができるまで ― お願いの合わせ技」に変更（PR #43）。本番確認済み
- [ ] 残り：D グラフ／E タブ区切り／F 並べ替え表／G .ics／H Skill 化（締め）。同じ手順（依頼文だけ渡す→無加工同梱→Playwright で確認）で
