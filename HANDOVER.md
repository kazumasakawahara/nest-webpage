# HANDOVER — 2026-07-16 main（番外編第2弾「共有フォルダ」draft完成・公開待ち）
...
- **共有フォルダ記事の残作業**：①「フォルダを指定する画面」「接続成功の画面」のスクショを河原さんが追加撮影・提供予定 → 提供後に figure 差し込み。本文の「追加の途中で、どのフォルダをAIに任せるかを指定する場面があります」の一文は**スクショ未確認のまま記述しており要検証**。②公開判断（draft: false 化）は河原さん。③コミットは main ローカルのみ・**push未実施**（pushすると自動デプロイされるが draft のため表には出ない）
...
- A: 共有フォルダ記事——追加スクショ（フォルダ指定・接続確認）の提供を受けて figure 差し込み＆該当文の検証 → 河原さんの公開判断で draft: false → push
