...
> 河原さん発案の常設コーナー。**うまくいった頼み方を、そのときの文面と返ってきたものごと一つずつ棚に並べる**。その1は「図（HTMLが良い）」の一言で、パソコン版 Google ドライブの同期方式の説明が、ボタンで切り替わる一枚のページになった実例。依頼文は河原さんの原文、お手本 HTML は AI 出力を無加工で同梱。**その1〜4 を PR #40・#41 で公開済み（2026-09-11）。次は D グラフから。候補は docs/ai-tips-kikaku-2026-09-11-onegai-ideas.md（A〜G＋締めの H「重なったら Skill に」）。**
...
- [x] その5「お願いの合わせ技」（河原さん提供の依頼文2本＝成年後見トピック記事の実録）＋看板挿絵の修正。ブランチにコミット、**push は「公開して」待ち**
