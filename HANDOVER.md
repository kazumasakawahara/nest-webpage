...
- 進捗:
  ...
  - [ ] nest News に特集完結の告知（過去の特集は公開時に告知記事あり。河原さん判断待ち）
  - [ ] 仕分け係 v1.1（#24 コメントの2点：source を `[[raw/…]]` に／テンプレートの穴埋め文字が未解決リンクになる）
...
## 次タスク（優先度順）
- A: PR #33 のマージと本番確認（記事3本・一覧「全3回」）
- B: nest News の告知記事を出すか決める（出すなら全3回の目次＋zip の案内）
- C: 仕分け係 v1.1（#24 の2点）と、Issue #24 を閉じる時期。記事本文の「正直」表現は今後使わない（メモリ no-shoujiki-phrasing）
